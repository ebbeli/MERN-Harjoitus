...
const createStory = async (req, res, next) => {
  const {story, date, city, picurl, user} = req.body;
      ...
      try {
          ...
          console.log(err)
          return next(new HttpError("Creating story failed, please try again! ;", 500));
      }
...
const getUsersStories = async (req, res, next)  => {
  ...
  try{
      ...
      return next(new HttpError("Could not retrieve story data, please try again!", 500));
  }
  if(!stories || stories === [] || stories === null){
    console.log("getUsersStores: Data could not been found(Maybe user does not have any)");
    return next(new HttpError('Stories could not been found(Maybe user does not have any)', 404));
  }
